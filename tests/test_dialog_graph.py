...
def test_reject_missing_child():
    with pytest.raises(ValueError) as excinfo:
        Dialog("ROOT",
               [DialogNode("ROOT", "::text::", NodeGraphics(image_ids=["::image::"]),
...
def test_reject_duplicate_ids():
    with pytest.raises(ValueError) as excinfo:
        Dialog("DUPLICATE", [DialogNode("DUPLICATE", "::text::", NodeGraphics(image_ids=["::image::"]), []),
...
def test_reject_missing_root():
    with pytest.raises(ValueError) as excinfo:
        Dialog("MISSING", [DialogNode("::id::", "::text::", NodeGraphics(image_ids=["::image::"]), [])])
    assert "No node found with ID: MISSING" in str(excinfo.value)


def test_initial_state():
    node = DialogNode("START", "Hello!", NodeGraphics(image_ids=["::image::"]), [DialogChoice("Good bye!", "START")])
    graph = Dialog("START", [node])
    assert graph.current_node() == node


def test_state_after_making_choice():
    first_node = DialogNode("START", "Hello!", NodeGraphics(image_ids=["::image::"]),
